...
Convexity implies that $F\geq \tfrac{F(1,\cdots,1)}{n} H$. The chosen normalization then implies,
\begin{equation}
\label{eq:speed_ineq}
...
\[
F^{kl}g_{kl} \leq n, \quad \text{ and } F^{kl,rs} \geq 0.
\]
Fixing an index \(i\) and working in normal coordinates centred on a critical point we have
\[
...
\]
from equation \eqref{eq:speed_ineq}. Note that for convex speeds, $c_s$ is automatically bounded. For concave speeds,
\[
c_s = \max w^i_i(\cdot, s) = \max \left(\frac{h^i_i(\cdot, s)}{F(\mathcal{W}(\cdot, s))} - \frac{1}{n}\right) \geq 0,
\]
again from equation \eqref{eq:speed_ineq}. For an upper bound however, we require the additional assumption, \(\tfrac{H}{F} \leq c\) so that \(c_s \leq \tfrac{1}{F} \sup k_{\max}(\cdot, s) \leq \tfrac{\sup H(\cdot, s)}{F} \leq c\).
...
\]
Hence, $H\equiv \kappa_{\max}$ and the flow is by totally umbilic hypersurfaces, and hence by geodesic spheres. In the concave case, all the inequalities are reversed and we obtain $H \equiv \kappa_{\min}$ and the same conclusion applies.
}
