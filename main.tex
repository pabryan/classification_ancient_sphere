...
\pf{
We may assume \(F\) is normalised so that \(F(1, \cdots, 1) = n\). Homogeneity and concavity implies that $F \leq \tfrac{F(1, \cdots, 1)}{n} H$, while homogeneity and convexity implies that $F\geq \tfrac{F(1,\cdots,1)}{n} H$. The chosen normalisation implies,
\begin{equation}
\label{eq:speed_ineq}
\begin{split}
