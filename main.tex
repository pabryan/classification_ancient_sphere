...
Denote by $C_{n}=C_{n}(\t)$ the unique great circle in $E,$ parameterized by arc length and containing $\pr(x_{n})$ and $\pr(y_{n})=\pr(R(x_n))$. Note that $C_n$ is well-defined for $n$ large enough; that is, $\pr(x_n)\ne \pr(R(x_n)),$ since $M_n$ is star-shaped and $x_n$ and $y_n$ are close to each other. 

The geodesics $C_n$ arise as the intersections of $E$ with some $2$-planes $Q_n;$ $C_n=E\cap Q_n.$ Consider the projection of $x_n$ to $E$,
$\pr(x_n)=\fr{\~x_n}{|\~x_n|},$
where $\~x_n=x_n-\inpr{x_n}{e}e.$ We have
\[\~x_n,~\widetilde{R(x_n)}\in Q_n.\]
Moreover, since
\[V=\fr{1}{2\inpr{x_n}{V}}\br{x_n-R(x_n)},\]
we also have $\~V\in Q_n$ and thus $\pr(V)\in C_n$ for all $n.$ 

Choose $p_n\in C_n\cap V^{\perp}$
such that the geodesic segment in $C_n$ that joins $\pr(x_n)$ to $p_n$ points into $H_V^-.$ Since
$\inpr{\pr(V)}{p_n}=0$
, $\pr(V)$ is tangent to $C_n$ at the point $p_n.$ Moreover, for all large $n,$ we have
\eq{\label{C_n}\fr{d}{d\t}\Big|_{\t_{p_n}}C_n=- \pr(V),}
where $\t_{p_n}$ is one of the angles with $C_n(\t_{p_n})=p_n.$

From the geodesics $C_n$ we will construct curves $\l_n$ and $\mu_n$ in $M_n$ and $R(M_n)$ respectively. The equator $E$ is diffeomorphic to $M_n$ via the map
\eq{X_n\cn E&\ra M_n\\
		\zeta&\mt \cos\br{f_{M_n}}\zeta+\sin\br{f_{M_n}}e.}
Define
\eq{\label{lambda} \l_n=X_n\circ C_n.}
Then
...
is a diffeomorphism as well. After a possible translation $\t\mt\t+\t_0,$ we may assume for angles $\t_1^n<0<\t_2^n$ that
...
Now
\[\xi_n(0)=\inpr{R\circ\l_n(\t^n_1+\t^n_2)}{e}-\inpr{\l_n(0)}{e}\ra 0,\quad n\ra\8,\]
...
\eq{\xi_n'(0)&=\inpr{\fr{d}{d\t}R\circ\l_n(\t^n_1+\t^n_2-\t)}{e}_{|\t=0}-\inpr{\l_n'(0)}{e}\\
 ...
 from \eqref{Conv} we obtain that one of the angles $\t_{p_n}$ converges to zero and for this one we have
\eq{\label{slope}|\l_n'(\t^n_1+\t^n_2)-C_n'(\t_{p_n})|&\leq |\l_n'(\t^n_1+\t^n_2)-C_n'(\t^n_1+\t^n_2)|\\
				&\hp{=}+|C_n'(\t^n_1+\t^n_2)-C_n'(0)|+|C_n'(0)-C_n'(\t_{p_n})|,}
which converges to zero due to \eqref{lambdaConv} and the assumption $H_n\leq \L.$ Since $C_n'(\t_{p_n})=\pm\pr(V),$ independently of $n,$ we obtain
\eq{\l_n'(\t^n_1+\t^n_2)\ra -\pr(V).}
for otherwise $\l_n$ would point into $H_V^{+}$ for large $n,$ which is impossible due to the choice of $\t^n_1$ and $\t^n_2.$ This implies that $\xi_n'(0)$ is uniformly positive. 
