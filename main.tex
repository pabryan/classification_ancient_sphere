...
\pf{
We may assume \(F\) is normalised so that \(F(1, \cdots, 1) = n\). Homogeneity and convexity implies that $F \geq \tfrac{F(1, \cdots, 1)}{n} H$, while homogeneity and concavity implies that $F\leq \tfrac{F(1,\cdots,1)}{n} H$. The chosen normalisation implies,
\begin{equation}
\label{eq:speed_ineq}
\begin{split}
\frac{\kappa_{\max}}{F} &\geq \frac{\kappa_{\max}}{H} \geq \frac{1}{n} \quad \text{$F$ concave}
 \\
\frac{\kappa_{\min}}{F} &\leq \frac{\kappa_{\min}}{H} \leq \frac{1}{n} \quad \text{$F$ convex}
\end{split}
\end{equation}
...
Homogeneity, concavity/convexity and the normalisation \(F(1, \cdots, 1) = n\) imply that
\[
\begin{split}
F^{kl}g_{kl} \geq n, \quad F^{kl,rs} \leq 0, \quad \text{$F$ concave} \\
F^{kl}g_{kl} \leq n, \quad F^{kl,rs} \geq 0, \quad \text{$F$ convex}.
\end{split}
\]

Then, fixing an index \(i\) and working in normal coordinates centred on a critical point we have
\[
\frac{1}{F} F^{kl,rs} \nabla_i h_{kl} \nabla^i h_{rs} = \frac{1}{F} F^{kl,rs} \nabla_i h_{kl} \nabla_i h_{rs}
\]
is non-positive for $F$ concave and non-negative for $F$ convex. Thus at a spatial maximum for $F$ concave, or a spatial minimum for $F$ convex we have
\begin{align*}
\partial_t \br{w^i_i-\fr{1}{n}} &\leq \Box\br{w^i_i-\fr{1}{n}} - 2n\br{w^i_i-\fr{1}{n}} \quad \text{$F$ concave} \\
\partial_t \br{w^i_i-\fr{1}{n}} &\geq \Box\br{w^i_i-\fr{1}{n}} - 2n\br{w^i_i-\fr{1}{n}} \quad \text{$F$ convex} \\
\end{align*}
...
\begin{split}
\frac{\kappa_{\min}}{F} &\geq \frac{1}{n} \quad \text{$F$ concave} \\
\frac{\kappa_{\max}}{F} &\leq \frac{1}{n} \quad \text{$F$ convex}
\end{split}
\end{equation}
...
\[
\begin{split}
\frac{\kappa_{\min}}{F} &\leq \frac{\kappa_{\min}}{H} \leq \frac{1}{n} \leq \frac{\kappa_{\min}}{F} \quad \text{$F$ concave} \\
\frac{\kappa_{\max}}{F} &\geq \frac{\kappa_{\max}}{H} \leq \frac{1}{n} \leq \frac{\kappa_{\max}}{F} \quad \text{$F$ convex}
\end{split}
\]
...
\end{split}
\]
may also be obtained from the non-collapsing result of \cite[Theorem 1.1]{AndrewsHanLiWei:/2015}, which states that any ancient solution of the flow with a 1-homogeneous convex speed $F$ satisfies
\[
\frac{\kappa_{\min}}{F}(x,t)\geq \frac{1}{n}+C(s)e^{-2n(t-s)},
\]
where \(C(s)\) is from the proof above.

For concave speeds, the non-collapsing result states that any ancient solution of the flow with a 1-homogeneous concave speed $F$ satisfies
\[
\frac{\kappa_{\max}}{F}(x,t) \leq \frac{1}{n}+C(s)e^{-2n(t-s)},
\]
again with \(C(s)\) as in the proof above.
\end{rem}
