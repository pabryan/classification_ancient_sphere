...
\eq{\label{Conv}R(x_{n})\ra x\quad\text{and}\quad y_{n}\ra x.}
...
Let $p_n\in C_n\cap V^{\perp}$
be arbitrary. Then
$\inpr{\pr(V)}{p_n}=0$
and hence $\pr(V)$ is tangent to $C_n$ at the point $p_n.$ That is, for all large $n,$ we have
\eq{\label{C_n}\fr{d}{d\t}\Big|_{p_n}C_n=\pm \pr(V).}

Geodesics $C_n$ give rise to smooth curves $\l_{n}$ in $M_{n}\hra \R^{n+2}$ and $\mu_{n}$ in $R_{V}(M_{n}),$ just by restricting the graph functions of $M_{n}$ and $R_{V}(M_{n})$ to $C_{n}.$
...
Due to \eqref{Conv} and \eqref{C_n} there holds
$\xi_{n}'(0)=\inpr{\mu'_{n}(0)}{e}-\inpr{\l'_{n}(0)}{e}\geq c_{\d}>0,$
since the first term is close to the corresponding slope of $R(C_n(-\t))$ at a point $p_n\in C_n\cap V^{\perp}$ i.e., to
$$2\sin \d\inpr{\~V}{V}\equiv 2c_{\d}>0,$$
