...
\begin{enumerate}[(i)]
\item{symmetric},
\item{positive on $\G_{+}$, $F(0)=0$}  and
\item{monotone, i.e., for all $1\leq i\leq n$ we have
\[\fr{\del F}{\del \k_i}\geq 0.\]}
\end{enumerate}
\end{ass}
We emphasize that no homogeneity, convexity or concavity restrictions are placed on the speed - all we require is that the speed is parabolic and positive, symmetric (equivalently, the speed is invariant under isometries, defined for convex hypersurfaces and \emph{shrinks} strictly convex hypersurfaces). The assumption $F(0)=0$ ensures that equators are static solutions and that the choice of an \emph{outward} unit normal makes sense for all nontrivial solutions.
...
\item \label{main2}$F\in C^1(\bar\Gamma_{+}),$ and $M_t$ is weakly convex for all $t \in (-T, 0)$.
\end{enumerate}
\noindent
...
\label{lem:spherical_existence_time}
 (i)~Let $p\in(0,1)$ and consider \eqref{eq:CurvFlow} with speed \(F = f^p\) where \(f\) is strictly monotone, concave and 1-homogeneous. Then the strictly convex spherical solution exists only on a finite time interval \((-T_S,0)\) with \(0 < T_S < \infty\), collapses to a point at \(t=0\) and converges to an equator at \(t=-T_S\).
...
Let $x$ be a convex and quasi-ancient solution of \eqref{eq:CurvFlow} with backwards bounded mean curvature. Then there holds:
\begin{enumerate}
...
Let the closed and convex hypersurfaces $M_{t},$ $-T_{S}<t<0,$ satisfy the parabolic curvature flow equation \eqref{eq:CurvFlow} under either assumption (i) or (ii) in \cref{thm:main} and suppose
...
and the connection and the norm $|\cdot|$ are those belonging to the spherical metric $\s_{ij}.$ Since the Weingarten operator is invariant under ambient isometries, the reflected hypersurfaces $R_{V}(M_{t})$ with the corresponding graph functions $u$ satisfy the same curvature flow equation.
...
\[\dot{w}=\Phi(x,w,\nabla w,\nabla^2w)\]
is not straightforward; note to the restriction that $F$ is generally only defined on non-negative definite endomorphisms. In the following, assuming (\ref{MPReflection1}), we however obtain a contradiction.
